...
All users have password: test123''';
  ...
  Future<void> _generateTestLocations() async {
    setState(() {
      isLoading = true;
      testResult = 'Generating test locations...';
    });
...
• Skipped: $skippedCount existing locations
• Total Available: ${createdCount + skippedCount} locations

🏟️ GENERATED LOCATIONS (Within 100 miles of Edwardsville, IL):
${testLocationsData.map((data) => '• ${data['name']} - ${data['address']}').join('\n')}
...
These locations are now available in the Test Assigner workspace for creating games and templates.''';
  ...
  Future<void> _createTestFootballCrew() async {
    setState(() {
      isLoading = true;
      testResult = 'Creating 2 test Football Crews from existing officials...';
    });
    ...
    try {
      ...
      Future<String> createCrew(String crewName, List<String> memberNames,
        ...
        for (int i = 0; i < foundOfficials.length; i++) {
          if (officialIds[i] != crewChiefId) {
    ...
    return Column(
      children: [
        ...
        SizedBox(
          width: double.infinity,
          child: ElevatedButton(
            onPressed: _generateTestLocations,
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.deepOrange,
              foregroundColor: Colors.white,
            ),
            child: const Text('🏟️ Generate Test Locations'),
          ),
        ),
        const SizedBox(height: 12),
        SizedBox(
        ...
        SizedBox(
          width: double.infinity,
          child: ElevatedButton(
            onPressed: _createBulkSchedulesAndTeams,
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.deepOrange,
              foregroundColor: Colors.white,
            ),
  ...
  Future<void> _createBulkSchedulesAndTeams() async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Create Bulk Schedules & Teams'),
        content: const Text(
            'This will create 36 schedules and teams for local area schools (O\'Fallon, Edwardsville, Highland, etc.). Continue?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancel'),
    ...
    setState(() {
      isLoading = true;
      testResult = 'Creating bulk schedules and teams...';
    });

    try {
      // IMPORTANT: Ensure we're logged in as the assigner before creating schedules
      final db = await DatabaseHelper().database;
      final assignerUsers = await db.query(
        'users',
        where: 'email = ? AND scheduler_type = ?',
      ...
      final scheduleData = [
        ...
        {'schedule': 'O\'Fallon Freshmen', 'team': 'O\'Fallon Panthers'},
        {'schedule': 'O\'Fallon Sophomore', 'team': 'O\'Fallon Panthers'},
        {'schedule': 'Edwardsville Varsity', 'team': 'Edwardsville Tigers'},
        {'schedule': 'Edwardsville Sophomores', 'team': 'Edwardsville Tigers'},
        {'schedule': 'Edwardsville Freshmen', 'team': 'Edwardsville Tigers'},
        {'schedule': 'Highland Varsity', 'team': 'Highland Bulldogs'},
        {'schedule': 'Highland Freshmen', 'team': 'Highland Bulldogs'},
        {'schedule': 'Highland Sophomores', 'team': 'Highland Bulldogs'},
        {'schedule': 'Triad Varsity', 'team': 'Triad Knights'},
        {'schedule': 'Triad Freshmen', 'team': 'Triad Knights'},
        {'schedule': 'Triad Sophomores', 'team': 'Triad Knights'},
        {'schedule': 'Alton Varsity', 'team': 'Alton Redbirds'},
        {'schedule': 'Alton Freshmen', 'team': 'Alton Redbirds'},
        {'schedule': 'Alton Sophomores', 'team': 'Alton Redbirds'},
      ...
      StringBuffer result = StringBuffer();
      result.writeln('Creating ${scheduleData.length} schedules...\n');
      ...
      for (final data in scheduleData) {
        try {
          final schedule = await scheduleService.createSchedule(
            name: data['schedule']!,
            sportName: 'Football',
            homeTeamName: data['team']!,
          );

          if (schedule != null) {
            created++;
            result.writeln('✅ Created: ${data['schedule']} (${data['team']})');
          } else {
            skipped++;
            result.writeln('⏭️ Skipped (exists): ${data['schedule']}');
          }
        } catch (e) {
          result.writeln('❌ Failed: ${data['schedule']} - $e');
        }
      }

      result.writeln('\n=== SUMMARY ===');
      result.writeln('Created: $created schedules');
      result.writeln('Skipped: $skipped schedules');
      result.writeln('Total: ${scheduleData.length} schedules processed');
      ...
      setState(() {
        testResult = 'Error creating bulk schedules: $e';
        isLoading = false;
      });
    }
  }
