...
class _OfficialGameDetailsScreenState extends State<OfficialGameDetailsScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (routeArgs is Map<String, dynamic> &&
        routeArgs.containsKey('assignment')) {
    ...
    if (assignment.id == null) {
      print(
          'WARNING: Assignment ID is null for assignment: ${assignment.toString()}');
    }
  ...
  Future<void> _loadGameDetails() async {
    try {
      ...
      if (assignment.gameId != null) {
        ...
        final officials = await _assignmentRepo
            .getConfirmedOfficialsForGame(assignment.gameId!);

        // Get scheduler information
        final scheduler =
            await _assignmentRepo.getSchedulerForGame(assignment.gameId!);

        setState(() {
          otherOfficials = officials
              .where((official) => official['id'] != assignment.officialId)
              .toList();
                                ...
                                GestureDetector(
                                  onTap: () async {
                                    ...
                                    final otherOfficialProfile = {
                                      ...
                                      'name': official['name'],
                                      'email':
                                          '${official['name'].toLowerCase().replaceAll(' ', '.')}@email.com',
                                      'phone':
                                          '(555) ${(officialId * 123).toString().padLeft(7, '0').substring(0, 3)}-${(officialId * 456).toString().padLeft(4, '0')}',
                                      'location':
                                      ...
                                      'bio':
                                          'Experienced official committed to fair play and sportsmanship.',
                                      ...
                                      'joinedDate': DateTime(
                                          ...
                                          (officialId % 12) + 1,
                                          (officialId % 28) + 1),
                                      'showCareerStats': (officialId % 2) ==
                                          0, // Some officials show stats, others don't
                                      'schedulerEndorsements':
                                          0, // Will be loaded by the profile screen
            ...
            Column(
              children: [
                ...
                if (schedulerInfo!['phone'] != null) ...[
                  const SizedBox(height: 12),
                  _buildContactRow(
                      Icons.phone, 'Phone', schedulerInfo!['phone'] as String),
                ],
              ],
            ),
        ],
  ...
  Widget _buildActionButtons() {
    return Column(
      children: [
        ...
        const SizedBox(height: 12),
        if (assignment.id !=
            null) // Only show back out button if assignment has valid ID
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
  ...
  Future<void> _launchEmail(String email, String schedulerName) async {
    ...
    final subject = Uri.encodeComponent(
        'Game Assignment: ${assignment.sportName} - $gameTitle');
  ...
  Future<void> _launchSMS(String phone, String schedulerName) async {
    ...
    final message = Uri.encodeComponent(
        'Hi $schedulerName, this is regarding the ${assignment.sportName} game: $gameTitle');
  ...
  Future<void> _showBackOutDialog() async {
    ...
    String dateString = gameDate != null
        ...
        : 'TBD';
    String timeString =
        gameTime != null ? DateFormat('h:mm a').format(gameTime) : 'TBD';
    ...
    try {
      ...
      if (linkedAssignments.isNotEmpty) {
        ...
        if (result == true) {
          ...
          Navigator.pop(
              context, true); // Return true to indicate successful back out
        }
      } else {
