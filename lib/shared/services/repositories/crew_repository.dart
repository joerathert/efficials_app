...
class CrewRepository extends BaseRepository {
  ...
  Future<int> addCrewMember(
      int crewId, int officialId, String position, String? gamePosition) async {
    debugPrint(
        'Adding crew member - crewId: $crewId, officialId: $officialId, position: $position');
  ...
  Future<int> removeCrewMember(int crewId, int officialId) async {
    debugPrint(
        'Removing crew member - crewId: $crewId, officialId: $officialId');

    return await withTransaction((txn) async {
      final result = await txn.update('crew_members', {'status': 'inactive'},
  ...
  Future<List<CrewMember>> getCrewMembers(int crewId,
  ...
  Future<bool> checkMemberAssignments(int crewId, int officialId) async {
    debugPrint(
        'Checking for active assignments for crew member - crewId: $crewId, officialId: $officialId');
    final results = await rawQuery('''
      SELECT COUNT(*) as count
  ...
  Future<int> setCrewAvailability(
      int crewId,
      DateTime date,
      String status,
      TimeOfDay? startTime,
      ...
      int setById) async {
    debugPrint(
        'Setting crew availability - crewId: $crewId, date: $date, status: $status');
    final data = {
      'crew_id': crewId,
  ...
  Future<List<CrewAvailability>> getCrewAvailability(
      int crewId, DateTime startDate, DateTime endDate) async {
    final results = await rawQuery('''
  ...
  Future<bool> isCrewAvailable(
      int crewId, DateTime date, TimeOfDay time) async {
    final results = await rawQuery('''
  ...
  Future<int> createCrewAssignment(CrewAssignment assignment) async {
    debugPrint(
        'Creating crew assignment for crew ID: ${assignment.crewId}, game ID: ${assignment.gameId}');
    return await insert('crew_assignments', assignment.toMap());
  }

  Future<List<CrewAssignment>> getPendingCrewAssignments(
  ...
  Future<int> respondToCrewAssignment(
      int assignmentId, String status, String? notes, int crewChiefId) async {
    debugPrint(
        'Responding to crew assignment - ID: $assignmentId, status: $status, crewChief: $crewChiefId');
    final result = await update(
        ...
        'id = ? AND crew_chief_id = ?',
        [assignmentId, crewChiefId]);
  ...
  Future<List<Crew>> getAvailableCrewsForGame(
      int sportId, String level, DateTime gameDate, TimeOfDay gameTime) async {
    final results = await rawQuery('''
      SELECT c.*, ct.required_officials, ct.level_of_competition,
             s.name as sport_name, o.name as crew_chief_name,
             o.city as crew_chief_city, o.state as crew_chief_state,
  ...
  Future<int> createCrewInvitation(CrewInvitation invitation) async {
    debugPrint(
        'Creating crew invitation for crew ID: ${invitation.crewId}, official ID: ${invitation.invitedOfficialId}');

    try {
      // Check for existing pending invitation
      final hasPending =
          await hasInvitation(invitation.crewId, invitation.invitedOfficialId);
      if (hasPending) {
        throw Exception(
      ...
      final isMember = await rawQuery('''
        SELECT 1 FROM crew_members 
        WHERE crew_id = ? AND official_id = ? AND status = 'active'
      ''', [invitation.crewId, invitation.invitedOfficialId]);

      if (isMember.isNotEmpty) {
        throw Exception('Official is already a member of this crew');
      }

      // Check for any existing invitation (including cancelled/declined)
      final existingInvitation = await rawQuery('''
        SELECT id FROM crew_invitations 
        WHERE crew_id = ? AND invited_official_id = ?
      ''', [invitation.crewId, invitation.invitedOfficialId]);
      ...
      // Create new invitation if no existing one found
      return await insert('crew_invitations', invitation.toMap());
    } catch (e) {
      debugPrint('Error creating crew invitation: $e');
      if (e.toString().contains('SQLITE_CONSTRAINT_UNIQUE')) {
        throw Exception(
            'Cannot re-invite official at this time. Please try again.');
      }
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>?> getLastInvitationStatus(
      int crewId, int officialId) async {
    final results = await rawQuery('''
  ...
  Future<List<Crew>> getFilteredCrews({
    ...
    if (ihsaCertifications != null && ihsaCertifications.isNotEmpty) {
      ...
      debugPrint(
          '🎯 After IHSA certification filter: ${allCrews.length} crews');
    }
    ...
    if (competitionLevels != null && competitionLevels.isNotEmpty) {
      ...
      allCrews = allCrews.where((crew) {
        if (crew.competitionLevels == null || crew.competitionLevels!.isEmpty) {
          debugPrint(
              '❌ Crew "${crew.name}" excluded: no competition levels set');
          return false;
        }
        // Check if any of the crew's selected levels match the filter
        final hasMatch = crew.competitionLevels!
            .any((crewLevel) => competitionLevels.contains(crewLevel));
        debugPrint(
            '${hasMatch ? '✅' : '❌'} Crew "${crew.name}": levels=${crew.competitionLevels}, filter=$competitionLevels, match=$hasMatch');
        return hasMatch;
      }).toList();
      debugPrint(
          '🏆 After competition level filter: ${allCrews.length} crews (was $beforeCount)');
    }
    ...
    if (maxDistanceMiles != null && gameLocation != null) {
      ...
      for (final crew in allCrews) {
        final withinDistance =
            await _crewWithinDistance(crew, gameLocation, maxDistanceMiles);
        debugPrint(
            '${withinDistance ? '✅' : '❌'} Crew "${crew.name}": distance check = $withinDistance');
        if (withinDistance) {
      ...
      debugPrint(
          '📍 After distance filter: ${allCrews.length} crews (was $beforeCount)');
    }
    ...
    final finalCrews = allCrews.where((crew) {
      ...
      debugPrint(
          '${canHire ? '✅' : '❌'} Crew "${crew.name}": canBeHired=$canHire (active=${crew.isActive}, fullStaffed=${crew.isFullyStaffed})');
      return canHire;
    }).toList();
    debugPrint(
        '🚀 Final result: ${finalCrews.length} crews (was $beforeCount)');

    return finalCrews;
  }

  // Check if crew meets IHSA certification requirements (lowest common level)
  Future<bool> _crewMeetsIHSACertification(
      Crew crew, List<String> requiredCertifications) async {
  ...
  String _findLowestCertificationLevel(List<String> certifications) {
    ...
    for (int i = certificationHierarchy.length - 1; i >= 0; i--) {
      ...
      for (int j = i; j < certificationHierarchy.length; j++) {
        membersWithThisLevelOrHigher +=
            certCounts[certificationHierarchy[j]] ?? 0;
      }
  ...
  bool _certificationMeetsRequirement(
      String actualLevel, List<String> requiredLevels) {
  ...
  Future<bool> _crewWithinDistance(
      Crew crew, Map<String, dynamic> gameLocation, int maxMiles) async {
    try {
      debugPrint(
          '🗺️ Checking distance for crew "${crew.name}" (chief ID: ${crew.crewChiefId})');
      ...
      if (crewChiefAddress == null) {
        debugPrint(
            '⚠️ No crew chief address - allowing crew (TODO: require addresses)');
      ...
      debugPrint(
          '✅ Distance check passed (TODO: implement actual distance calculation)');
      return true;
    } catch (e) {
  ...
  Future<int> updateCrewCompetitionLevels(
      int crewId, List<String> competitionLevels) async {
