...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 2) {
      ...
      await db.execute(
          'ALTER TABLE officials ADD COLUMN official_user_id INTEGER REFERENCES official_users(id)');
      await db.execute('ALTER TABLE officials ADD COLUMN email TEXT');
      await db.execute('ALTER TABLE officials ADD COLUMN phone TEXT');
      await db.execute(
          'ALTER TABLE officials ADD COLUMN availability_status TEXT DEFAULT "available"');
      await db
          .execute('ALTER TABLE officials ADD COLUMN profile_image_url TEXT');
      await db.execute('ALTER TABLE officials ADD COLUMN bio TEXT');
      await db
          .execute('ALTER TABLE officials ADD COLUMN experience_years INTEGER');
      await db
          .execute('ALTER TABLE officials ADD COLUMN certification_level TEXT');
      await db.execute(
          'ALTER TABLE officials ADD COLUMN is_user_account BOOLEAN DEFAULT FALSE');

      // Add new columns to existing users table
      await db.execute(
          'ALTER TABLE users ADD COLUMN user_type TEXT DEFAULT "scheduler"');
      ...
      await db.execute(
          'CREATE INDEX idx_official_users_email ON official_users(email)');
      await db.execute(
          'CREATE INDEX idx_game_assignments_game_id ON game_assignments(game_id)');
      await db.execute(
          'CREATE INDEX idx_game_assignments_official_id ON game_assignments(official_id)');
      await db.execute(
          'CREATE INDEX idx_game_assignments_status ON game_assignments(status)');
      await db.execute(
          'CREATE INDEX idx_official_availability_official_id ON official_availability(official_id)');
      await db.execute(
          'CREATE INDEX idx_official_availability_date ON official_availability(date)');
      await db.execute(
          'CREATE INDEX idx_official_sports_official_id ON official_sports(official_id)');
      await db.execute(
          'CREATE INDEX idx_official_sports_sport_id ON official_sports(sport_id)');
      await db.execute(
          'CREATE INDEX idx_official_notifications_official_id ON official_notifications(official_id)');
      await db.execute(
          'CREATE INDEX idx_official_settings_official_id ON official_settings(official_id)');
    }
    ...
    if (oldVersion < 3) {
      ...
      try {
        ...
        if (currentSchoolName != null &&
            ...
            currentSchoolName != 'Default School' &&
            currentSchoolName != 'Central High School') {
    ...
    if (oldVersion < 5) {
      ...
      try {
        ...
        if (schoolName != null && mascot != null) {
          ...
          print(
              'Successfully migrated AD school info from SharedPreferences to database');
        }
      } catch (e) {
    ...
    if (oldVersion < 7) {
      ...
      await db.execute(
          'ALTER TABLE game_assignments ADD COLUMN backed_out_at DATETIME');
      await db.execute(
          'ALTER TABLE game_assignments ADD COLUMN back_out_reason TEXT');
    }
    ...
    if (oldVersion < 9) {
      ...
      await db.execute(
          'ALTER TABLE officials ADD COLUMN follow_through_rate DECIMAL(5,2) DEFAULT 100.0');
      await db.execute(
          'ALTER TABLE officials ADD COLUMN total_accepted_games INTEGER DEFAULT 0');
      await db.execute(
          'ALTER TABLE officials ADD COLUMN total_backed_out_games INTEGER DEFAULT 0');
      ...
      await db.execute(
          'CREATE INDEX idx_backout_notifications_official_id ON official_backout_notifications(official_id)');
      await db.execute(
          'CREATE INDEX idx_backout_notifications_scheduler_id ON official_backout_notifications(scheduler_id)');
      await db.execute(
          'CREATE INDEX idx_backout_notifications_assignment_id ON official_backout_notifications(assignment_id)');
      await db.execute(
          'CREATE INDEX idx_backout_notifications_excused_at ON official_backout_notifications(excused_at)');

      // Add excuse-related columns to game_assignments table
      await db.execute(
          'ALTER TABLE game_assignments ADD COLUMN excused_backout BOOLEAN DEFAULT FALSE');
      await db.execute(
          'ALTER TABLE game_assignments ADD COLUMN excused_at DATETIME');
      await db.execute(
          'ALTER TABLE game_assignments ADD COLUMN excused_by INTEGER REFERENCES users(id)');
      await db.execute(
          'ALTER TABLE game_assignments ADD COLUMN excuse_reason TEXT');
    }
    ...
    if (oldVersion < 10) {
      ...
      // Create indexes for performance
      await db.execute(
          'CREATE INDEX idx_endorsements_endorsed_official_id ON official_endorsements(endorsed_official_id)');
      await db.execute(
          'CREATE INDEX idx_endorsements_endorser_user_id ON official_endorsements(endorser_user_id)');
      await db.execute(
          'CREATE INDEX idx_endorsements_endorser_type ON official_endorsements(endorser_type)');
    }
    ...
    if (oldVersion < 16) {
      // Add selected_lists column to game_templates table
      await db
          .execute('ALTER TABLE game_templates ADD COLUMN selected_lists TEXT');
    }
  ...
  Future<void> _createIndexes(Database db) async {
    ...
    await db
        .execute('CREATE INDEX idx_games_schedule_id ON games(schedule_id)');
    ...
    await db
        .execute('CREATE INDEX idx_schedules_user_id ON schedules(user_id)');
    await db
        .execute('CREATE INDEX idx_locations_user_id ON locations(user_id)');
    await db
        .execute('CREATE INDEX idx_officials_user_id ON officials(user_id)');
  ...
  Future<void> migrateFromSharedPreferences() async {
    ...
    final migrationCompleted =
        prefs.getBool('database_migration_completed') ?? false;
  ...
  Future<void> _migrateUserData(
      Transaction txn, SharedPreferences prefs) async {
  ...
  Future<void> _migrateGames(Transaction txn, SharedPreferences prefs) async {
    ...
    await _migrateGamesList(
        txn, prefs, 'assigner_published_games', 'Published');

    // Migrate unpublished games
    await _migrateGamesList(txn, prefs, 'ad_unpublished_games', 'Unpublished');
    await _migrateGamesList(
        txn, prefs, 'coach_unpublished_games', 'Unpublished');
    await _migrateGamesList(
        txn, prefs, 'assigner_unpublished_games', 'Unpublished');
  }

  Future<void> _migrateGamesList(Transaction txn, SharedPreferences prefs,
      String key, String status) async {
    ...
    try {
      ...
      for (var game in games) {
        // Get or create sport
        final sportId =
            await _getOrCreateSport(txn, game['sport'] ?? 'Football');

        // Get or create schedule
        final scheduleId =
            await _getOrCreateSchedule(txn, game['scheduleName'], sportId);
  ...
  Future<int?> _getOrCreateSchedule(
      Transaction txn, String? scheduleName, int sportId) async {
  ...
  Future<int?> _getOrCreateLocation(
      Transaction txn, String? locationName) async {
  ...
  Future<void> _migrateTemplates(
    ...
    try {
      ...
      for (var template in templates) {
        final sportId =
            await _getOrCreateSport(txn, template['sport'] ?? 'Football');
        final locationId =
            await _getOrCreateLocation(txn, template['location']);
        ...
        await txn.insert('game_templates', {
          ...
          'include_level_of_competition':
              template['includeLevelOfCompetition'] ?? false,
          'include_gender': template['includeGender'] ?? false,
          'include_officials_required':
              template['includeOfficialsRequired'] ?? false,
          'include_game_fee': template['includeGameFee'] ?? false,
          'include_opponent': template['includeOpponent'] ?? false,
          'include_hire_automatically':
              template['includeHireAutomatically'] ?? false,
          'include_selected_officials':
              template['includeSelectedOfficials'] ?? false,
  ...
  Future<void> _createCrewIndexes(Database db) async {
    await db.execute(
        'CREATE INDEX idx_crew_types_sport_level ON crew_types(sport_id, level_of_competition)');
    await db
        .execute('CREATE INDEX idx_crews_crew_chief ON crews(crew_chief_id)');
    ...
    await db.execute(
        'CREATE INDEX idx_crew_members_official_id ON crew_members(official_id)');
    await db.execute(
        'CREATE INDEX idx_crew_availability_crew_date ON crew_availability(crew_id, date)');
    await db.execute(
        'CREATE INDEX idx_crew_assignments_game_id ON crew_assignments(game_id)');
    await db.execute(
        'CREATE INDEX idx_crew_assignments_crew_chief ON crew_assignments(crew_chief_id)');
    await db.execute(
        'CREATE INDEX idx_crew_assignments_status ON crew_assignments(status)');
    await db.execute(
        'CREATE INDEX idx_crew_payments_assignment ON crew_payment_distributions(crew_assignment_id)');
  }
    ...
    final defaultCrewTypes = [
      {
        ...
        'officials': 5,
        'desc':
            'Varsity Football - 5 Officials (Referee, Umpire, Head Linesman, Line Judge, Back Judge)'
      },
      {
        'sport': 'Football',
        'level': 'Underclass',
        'officials': 4,
        'desc': 'JV/Freshman Football - 4 Officials'
      },
      {
        'sport': 'Baseball',
        'level': 'All',
        'officials': 2,
        'desc': 'All Baseball Levels - 2 Officials (Home Plate, Base Umpire)'
      },
      {
        'sport': 'Basketball',
        'level': 'Varsity',
        'officials': 3,
        'desc': 'Varsity Basketball - 3 Officials'
      },
      {
        'sport': 'Basketball',
        'level': 'JV',
        'officials': 3,
        'desc': 'JV Basketball - 3 Officials'
      },
      {
        'sport': 'Basketball',
        'level': 'Other',
        'officials': 2,
        'desc': 'Freshman/Middle School Basketball - 2 Officials'
      },
    ];
  ...
  Future<void> _addCrewInvitationsTable(Database db) async {
    ...
    // Create indexes for performance
    await db.execute(
        'CREATE INDEX idx_crew_invitations_crew_id ON crew_invitations(crew_id)');
    await db.execute(
        'CREATE INDEX idx_crew_invitations_invited_official_id ON crew_invitations(invited_official_id)');
    await db.execute(
        'CREATE INDEX idx_crew_invitations_status ON crew_invitations(status)');
  ...
  Future<void> _addMissingNotificationTables(Database db) async {
    try {
      ...
      final notificationSettingsTableExists =
          await _tableExists(db, 'notification_settings');
    ...
    try {
      ...
      if (result.isNotEmpty) {
        ...
        if (tableSchema.contains(
            "('backout', 'game_filling', 'official_interest', 'official_claim')")) {
          print(
              'Updating notifications table schema to support official_removal type...');
          ...
          print(
              '✅ Notifications table updated successfully with official_removal type');
        } else {
  ...
  Future<void> _addAdvancedMethodTables(Database db) async {
    try {
      print('Adding Advanced Method (Multiple Lists) tables...');

      // Game List Quotas table - tracks quotas for each list per game
      await db.execute('''
        CREATE TABLE IF NOT EXISTS game_list_quotas (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          game_id INTEGER NOT NULL REFERENCES games(id) ON DELETE CASCADE,
          list_id INTEGER NOT NULL REFERENCES official_lists(id) ON DELETE CASCADE,
          min_officials INTEGER NOT NULL DEFAULT 0,
          max_officials INTEGER NOT NULL DEFAULT 0,
          current_officials INTEGER NOT NULL DEFAULT 0,
          created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
          updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      ...
      // Official List Assignments table - tracks which list an official was assigned from
      await db.execute('''
        CREATE TABLE IF NOT EXISTS official_list_assignments (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          game_id INTEGER NOT NULL REFERENCES games(id) ON DELETE CASCADE,
      ...
      // Create indexes for performance
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_game_list_quotas_game_id ON game_list_quotas(game_id)');
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_game_list_quotas_list_id ON game_list_quotas(list_id)');
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_game_list_quotas_game_list ON game_list_quotas(game_id, list_id)');
      
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_official_list_assignments_game_id ON official_list_assignments(game_id)');
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_official_list_assignments_official_id ON official_list_assignments(official_id)');
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_official_list_assignments_list_id ON official_list_assignments(list_id)');
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_official_list_assignments_game_official ON official_list_assignments(game_id, official_id)');
